...
PetscReal piecewise(PetscReal rho, PetscReal beta){
...
int main(int argc, char **argv)
{
  ...
  PetscInt printfiltered;
  getint("-printfiltered",&printfiltered,0);
  if(printfiltered==1){
    PetscReal *dof_filt = (PetscReal *)malloc(ndof*sizeof(PetscReal));
    for(PetscInt i=0;i<ndof;i++)
      dof_filt[i]=piecewise(dof[i],filter_beta);
    char strfilt[PETSC_MAX_PATH_LEN];
    sprintf(strfilt,"%s.filtered.txt",strin);
    writetofile(PETSC_COMM_WORLD, strfilt, dof_filt, REAL, ndof);
    free(dof_filt);
  }
