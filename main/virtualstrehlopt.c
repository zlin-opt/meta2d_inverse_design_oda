...
int main(int argc, char **argv)
{
  ...
  for(PetscInt isim_per_comm=0;isim_per_comm<nsims_per_comm;isim_per_comm++){
    ...
    PetscInt virtual_aperture=round(nx*ncells*foclen/virtual_foclen);
    PetscInt virtual_aperture_diff=(nx*ncells-virtual_aperture)/2;
    PetscInt local_start = px + ( (icell==0) ? virtual_aperture_diff : 0 );
    PetscInt local_end = px + nx - ( (icell==ncells-1) ? virtual_aperture_diff : 0 );
    PetscInt local_span = local_end - local_start;
    PetscInt global_start;
    if(icell==0)
      global_start = 0;
    else
      global_start = virtual_aperture_diff + nx*(icell-1);
    ...
    gforms(subcomm, ge[isim_per_comm], gh[isim_per_comm],
	   ...
	   dx,dz,
	   omega,
	   local_start,local_span,global_start,jz_mtr,
	   fspotx,fspotz,
	   origx,origz);
  ...
  for(PetscInt iangle=0;iangle<nangles;iangle++){
    for(PetscInt ifreq=0;ifreq<nfreqs;ifreq++){
      ...
      PetscReal NA = sin(atan(Nx*dx/(2*virtual_foclen)));
  ...
  if(Job==0){
    ...
    srcJy(Jsrc, 0,odmNx,dx,dz, kwav0,alpha0,1);
    ...
    for(PetscInt jx=0;jx<Nxfar;jx++){
      PetscReal xfar = fspotx + jx*dx - Nxfar*dx/2.0;
      PetscReal zfar = fspotz;
